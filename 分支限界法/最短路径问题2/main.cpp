...
struct Node{
    ...
    int _leftValue;
    int _way[MAX];
    Node(int id,int curValue,int leftValue)
    {
        _id=id;
        _curValue=curValue;
    ...
    bool operator <(const Node& node)const{return _curValue+_leftValue>node._curValue+node._leftValue;}
};
...
int main(int argc,char **argv)
{
    ...
    for(int i=1;i<=totalNum;i++){
        ...
        if(temp==INF){
            cout<<"The map is seperator."<<endl;
            exit(0);
        }
        minTotal+=temp;
        minWay[i]=temp;
    ...
    while(Q.empty()!=true){
        ...
        for(int i=t;i<=totalNum;i++){           
            if(relation[t-1][i]!=INF){
                node._curValue+=relation[node._way[t-1]][node._way[i]];
                node._leftValue-=minWay[node._way[t]];                
